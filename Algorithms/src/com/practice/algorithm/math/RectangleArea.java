...
public class RectangleArea {
    ...
    public static void main(String[] args) {
        ...
        int area = ra.computeArea(-3, 0, 3, 4, 0, -1, 9, 2);
    ...
    public int computeArea(int ax1, int ay1, int ax2, int ay2, int bx1, int by1, int bx2, int by2) {
        ...
        //no overlap rectangle
        if (ax2 <= bx1 || bx2 <= ax1 || ay2 <= by1 || by2 <= ay1) {
            return aA + bA;
        }

        //find the overlapping rectangle points
        int overlapLeft = Math.max(ax1, bx1);
        int overlapRight = Math.min(ax2, bx2);
        int overlapTop = Math.max(ay1, by1);
        int overlapBottom = Math.min(ay2, by2);

        int oh = overlapRight - overlapLeft;
        int ol = overlapBottom - overlapTop;
        int oA = oh * ol;
        return aA + bA - oA;
    }
